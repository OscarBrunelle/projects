...
public class GameView extends JFrame {
    ...
    private JLabel flaggingLabel = new JLabel("Flag Mode: Not Activated");
    ...
    public GameView(GameModel gameModel, GameController gameController) {
        ...
        JButton setFlagMode = new JButton("Set Flag Mode");
        setFlagMode.addActionListener(gameController);
        ...
        buttons.add(setFlagMode);
        buttons.add(flaggingLabel);
        buttons.add(reset);
        buttons.add(exit);
    ...
    public void update(){
        ...
        for (int i=0; i<h; i++) {
            for (int j=0; j<w; j++) {
                if (gameModel.isFlagged(i,j)) {
                    board[i][j].setIconNumber(DotButton.FLAGGED);
                }
                else if (gameModel.isMined(i,j) && gameModel.hasBeenClicked(i,j) && !gameModel.isFlagged(i,j)) {
                    board[i][j].setIconNumber(DotButton.CLICKED_MINE);
                }
                else if(gameModel.hasBeenClicked(i,j) || !gameModel.isCovered(i,j) && !gameModel.isFlagged(i,j)){
                    icon=getIcon(i,j);
        ...
        if (gameController.getFlagMode()) {
            flaggingLabel.setText("Flag Mode: Activated");
        } else {
            flaggingLabel.setText("Flag Mode: Not Activated");
        }
